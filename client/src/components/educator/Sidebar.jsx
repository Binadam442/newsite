...
const Sidebar = () => {
  ...
  const menuItems = [
    ...
    { name: "Students Enrolled", path: "/educator/student-enroled", icon: assets.person_tick_icon },
  ];
  ...
  return (
    isEducator && (
      <div className="md:w-64 w-16 border-r min-h-screen text-base border-gray-500 py-2 flex flex-col">
        {menuItems.map((item) => (
          <NavLink
            to={item.path} // ✅ Fixed navigation path
            key={item.name}
            end={item.path === "/educator"} // ✅ Ensures active class works for exact match
            className={({ isActive }) =>
